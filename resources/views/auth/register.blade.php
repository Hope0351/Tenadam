            ...
            <form method="POST" action="{{ url('/register') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-sm-7 mb-4">
                        ...
                        <br>
                        {{ Form::tel('phone', old('phone') ?? getCountryCode(), ['class' => 'form-control phoneNumber', 'id' => 'phone', 'onkeyup' => 'this.value=this.value.replace(/\D/g,"")']) }}
                        {{ Form::hidden('prefix_code', old('prefix_code') ?? getCountryCode(), ['class' => 'prefix_code']) }}
    ...
    <script>
        $(document).ready(function() {
            ...
            function setPrefixCode() {
                let code = intl.getSelectedCountryData().dialCode;
                $('.prefix_code').val(code);
            }

            let oldPhone = "{{ old('phone') }}";
            let oldPrefix = "{{ old('prefix_code') }}";

            if (oldPhone) {
                intl.setNumber('+' + oldPrefix + oldPhone);
            }
