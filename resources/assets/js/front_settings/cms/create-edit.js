...
function loadFrontSettingCMSData() {
    ...
    if (
        ...
        listenSubmit("#termsAndCondition", function () {
            ...
            if (quill1.getText().trim().length === 0) {
                displayErrorMessage(
                    Lang.get("js.the_terms_conditions_is_required"),
                );
                return false;
            }
            ...
            if (quill2.getText().trim().length === 0) {
                displayErrorMessage(
                    Lang.get("js.the_privacy_policy_is_required"),
                );
                return false;
            }
...
listenSubmit("#addCMSForm", function () {
    ...
    if (isEmpty($.trim(homeDoctorDescription))) {
        displayErrorMessage(
            Lang.get("js.home_page_certified_doctor_description_required"),
        );
        return false;
    }
    if (isEmpty($.trim(homePageCerBoxTl))) {
        displayErrorMessage(
            Lang.get("js.home_page_certified_box_title_required"),
        );
        return false;
    }
    if (isEmpty($.trim(homePageCerBoxDes))) {
        displayErrorMessage(
            Lang.get("js.home_page_certified_description_required"),
        );
        return false;
    }
    ...
    if (isEmpty($.trim(homePageStep2Des))) {
        displayErrorMessage(
            Lang.get("js.home_page_step_two_description_required"),
        );
        return false;
    }
    ...
    if (isEmpty($.trim(homePageStep3Des))) {
        displayErrorMessage(
            Lang.get("js.home_page_step_three_description_required"),
        );
        return false;
    }
    ...
    if (isEmpty($.trim(homePageStep4Des))) {
        displayErrorMessage(
            Lang.get("js.home_page_step_four_description_required"),
        );
        return false;
    }
...
listenSubmit("#createAboutUs", function () {
    ...
    if (isEmpty($.trim(aboutUsDes))) {
        displayErrorMessage(Lang.get("js.about_us_description_required"));
        return false;
    }
    if (isEmpty($.trim(aboutUsMission))) {
        displayErrorMessage(Lang.get("js.about_us_mission_required"));
        return false;
    }
});

listenSubmit("#addLocationMap", function () {
    let locationIframe = $("#locationIframe").val().trim();

    if (isEmpty(locationIframe)) {
        displayErrorMessage(Lang.get("js.google_map_required"));
        return false;
    }

    let iframeRegex = /<iframe[\s\S]*?>[\s\S]*?<\/iframe>/i;

    if (!iframeRegex.test(locationIframe)) {
        displayErrorMessage(Lang.get("js.invalid_iframe_code"));
        return false;
    }
